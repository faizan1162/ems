...
class DashboardService implements DashboardInterface
{
    ...
    public function getYearlySales($year = 2022)
    {
        ...
        $sql = "SELECT SUM(`total_price`) AS `total_earnings`, MONTH(created_at) AS `sales_month` FROM event_ticket_bookings WHERE YEAR(`created_at`) = '{$year}' GROUP BY sales_month ORDER BY MONTH(created_at) ASC";
